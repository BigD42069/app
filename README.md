...
| iOS       | `ios/Frameworks/ios.xcframework` | Das Xcode-Projekt referenziert das Framework jetzt relativ zum Repository-Wurzelpfad (`path = ios/Frameworks/ios.xcframework`). Kopiere den kompletten `ios.xcframework`-Ordner – inklusive der Unterordner `ios-arm64` und `ios-arm64_x86_64-simulator` samt `Mobile.framework` – dort hinein, bevor du Xcode startest. |
