...
|-----------|-----------------|----------|
| Android   | `android/app/libs/mobile.aar` | Die `android/app/build.gradle.kts` lädt das AAR direkt aus diesem Verzeichnis via `implementation(files("libs/mobile.aar"))`. Du musst die Datei genau so benennen und lokal einspielen. |
| iOS       | `ios/Frameworks/Mobile.xcframework` | Das Xcode-Projekt referenziert das Framework relativ zum `Frameworks/`-Ordner (`path = ../Frameworks/Mobile.xcframework`). Kopiere den kompletten `Mobile.xcframework`-Ordner dort hinein, bevor du Xcode startest. |
