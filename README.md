...
| iOS       | `ios/Frameworks/Mobile.xcframework` | Das Xcode-Projekt referenziert das Framework jetzt relativ zum Repository-Wurzelpfad (`path = Frameworks/Mobile.xcframework` in Xcode). Kopiere den kompletten `Mobile.xcframework`-Ordner – inklusive der Unterordner `ios-arm64` und `ios-arm64_x86_64-simulator` samt `Mobile.framework` und Header-Dateien – dort hinein, bevor du Xcode startest. |
